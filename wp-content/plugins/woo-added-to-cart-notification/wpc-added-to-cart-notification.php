...
! defined( 'WOOAC_VERSION' ) && define( 'WOOAC_VERSION', '2.3.3' );
		...
		if ( ! class_exists( 'WPCleverWooac' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWooac {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WOOAC_FILE );
						}
					} );
				}
                                ...
                                <form method="post" action="options.php">
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="regular-text" name="wooac_localization[related]" value="<?php echo esc_attr( self::localization( 'related' ) ); ?>" placeholder="<?php esc_attr_e( 'You may also like', 'woo-added-to-cart-notification' ); ?>"/>
                                            </td>
                                        </tr>
                                        <tr>
				...
				function enqueue_scripts() {
					...
					wp_localize_script( 'wooac-frontend', 'wooac_vars', [
				...
				function get_notiny() {
					...
					echo '<div class="wooac-wrapper wooac-notiny">';
				...
				function get_popup() {
					...
					echo '<div class="wooac-wrapper wooac-popup mfp-with-anim">';
					do_action( 'wooac_wrap_before' );
					//echo apply_filters( 'wooac_before', '' );
						...
						if ( $product && ( $product_id = $product->get_id() ) && ! apply_filters( 'wooac_exclude', false, $product, $item ) ) {
							if ( ! in_array( $product_id, apply_filters( 'wooac_exclude_ids', [ 0 ] ), true ) ) {
								...
								if ( self::get_setting( 'show_image', 'yes' ) === 'yes' ) {
									...
									//echo apply_filters( 'wooac_image_before', '' );
									...
									//echo apply_filters( 'wooac_image_after', '' );
									do_action( 'wooac_image_after' );
								}

								do_action( 'wooac_text_before' );
								//echo apply_filters( 'wooac_text_before', '' );
								...
								if ( self::get_setting( 'show_content', 'yes' ) === 'yes' ) {
									...
									//echo apply_filters( 'wooac_cart_content_after', '' );
								...
								if ( ( ( self::get_setting( 'show_share_cart', 'yes' ) === 'yes' ) && class_exists( 'WPCleverWpcss' ) ) || ( self::get_setting( 'show_view_cart', 'yes' ) === 'yes' ) || ( self::get_setting( 'show_checkout', 'no' ) === 'yes' ) || ( self::get_setting( 'show_continue_shopping', 'yes' ) === 'yes' ) ) {
									...
									//echo apply_filters( 'wooac_action_before', '' );
									...
									//echo apply_filters( 'wooac_action_after', '' );
					...
					//echo apply_filters( 'wooac_after', '' );
					do_action( 'wooac_wrap_after' );
					echo '</div>';
					do_action( 'wooac_wrap_below' );

					return apply_filters( 'wooac_popup_html', ob_get_clean() );
				}
